defmodule GitTest do
  ...
  # ---------------------------------------------------------------------------
  # Git.Status.parse/1 unit tests
  # ---------------------------------------------------------------------------

  describe "Git.Status.parse/1" do
    test "parses empty output" do
      result = Git.Status.parse("")
      assert %Git.Status{branch: nil, entries: []} = result
    end

    test "parses clean repo on main branch" do
      output = "## main\n"
      result = Git.Status.parse(output)
  ...
  # ---------------------------------------------------------------------------
  # Git.Commands.Log args/1 unit tests
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # Git.Commands.Commit args/1 unit tests
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # Git.Commands.Commit parse_output/2 unit tests
  # ---------------------------------------------------------------------------

  describe "Git.Commands.Commit parse_output/2" do
    test "returns ok with CommitResult on exit 0" do
      output =
        "[main abc1234] the commit message\n 1 file changed, 5 insertions(+), 2 deletions(-)\n"

      assert {:ok, %Git.CommitResult{} = result} = Commit.parse_output(output, 0)
      assert result.branch == "main"
  ...
  # ---------------------------------------------------------------------------
  # Git.status/1 integration tests
  # ---------------------------------------------------------------------------

  describe "Git.status/1" do
    test "returns ok status struct on clean repo" do
      ...
      assert {:ok, %Git.Status{entries: []}} = Git.status(config: config)
    end
  ...
  # ---------------------------------------------------------------------------
  # Git.log/1 integration tests
  # ---------------------------------------------------------------------------

  describe "Git.log/1" do
    ...
    test "returns commit struct after one commit" do
      ...
      assert {:ok, [commit]} = Git.log(config: config)
    ...
    test "respects max_count option" do
      ...
      assert {:ok, commits} = Git.log(max_count: 2, config: config)
    ...
    test "returns multiple commits in reverse chronological order" do
      ...
      assert {:ok, [latest, oldest]} = Git.log(config: config)
  ...
  describe "module" do
    test "defines status/1" do
      assert function_exported?(Git, :status, 1)
    end

    test "defines log/1" do
      assert function_exported?(Git, :log, 1)
    end

    test "defines commit/2" do
      assert function_exported?(Git, :commit, 2)
    end
  end
end
