defmodule Git.Commands.CloneTest do
  use ExUnit.Case, async: true

  alias Git.Commands.Clone
  alias Git.Config
  ...
  describe "clone integration" do
    test "clones a local repo into the working directory", %{
      ...
      assert {:ok, :done} = Git.clone(src_dir, config: config)
    ...
    test "clones into a custom directory name", %{
      ...
      assert {:ok, :done} = Git.clone(src_dir, directory: "custom-name", config: config)
    ...
    test "clones with --depth=1 (shallow clone)", %{src_dir: src_dir, config: config} do
      assert {:ok, :done} = Git.clone(src_dir, depth: 1, config: config)
    end

    test "clones with --branch flag", %{src_dir: src_dir, config: config} do
      assert {:ok, :done} = Git.clone(src_dir, branch: "main", config: config)
    end
