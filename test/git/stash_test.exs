defmodule Git.StashTest do
  ...
  describe "StashEntry.parse/1" do
    ...
    test "parses a single stash entry with explicit message" do
      ...
      assert [%StashEntry{index: 0, branch: "main", message: "my changes"}] =
    ...
    test "parses a WIP stash entry" do
      ...
      assert [%StashEntry{index: 0, branch: "main", message: "abc1234 commit message"}] =
               StashEntry.parse(output)
    end
  ...
  describe "Commands.Stash.args/1" do
    ...
    test "save with message" do
      assert StashCmd.args(%StashCmd{save: true, message: "wip"}) == [
               "stash",
               "push",
               "-m",
               "wip"
    ...
    test "save with include_untracked" do
      assert StashCmd.args(%StashCmd{save: true, include_untracked: true}) == [
               "stash",
               "push",
               "-u"
             ]
  ...
  describe "Git.stash/1 integration" do
    ...
    test "save stashes tracked changes" do
      ...
      assert {:ok, :done} = Git.stash(config: config, save: true)

      # Working tree should be clean after stash
      assert {:ok, status} = Git.status(config: config)
    ...
    test "save with message" do
      ...
      assert {:ok, :done} = Git.stash(config: config, save: true, message: "my stash")
    ...
    test "saved stash appears in listing" do
      {config, dir} = setup_repo()
      File.write!(Path.join(dir, "init.txt"), "modified\n")
      Git.stash(config: config, save: true, message: "test stash")
    ...
    test "pop restores stashed changes" do
      ...
      Git.stash(config: config, save: true)

      assert {:ok, :done} = Git.stash(config: config, pop: true)
    ...
    test "drop removes stash entry" do
      {config, dir} = setup_repo()
      File.write!(Path.join(dir, "init.txt"), "modified\n")
      Git.stash(config: config, save: true, message: "to drop")

      assert {:ok, :done} = Git.stash(config: config, drop: true, index: 0)
    ...
    test "multiple stashes maintain correct ordering" do
      ...
      Git.stash(config: config, save: true, message: "first stash")

      File.write!(file_path, "second\n")
      Git.stash(config: config, save: true, message: "second stash")
    ...
    test "pop with specific index" do
      ...
      assert {:ok, :done} = Git.stash(config: config, pop: true, index: 1)
