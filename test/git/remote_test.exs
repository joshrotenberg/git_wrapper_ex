defmodule Git.RemoteTest do
  ...
  defp setup_repo do
    ...
    Git.Config.new(working_dir: dir)
  end
  ...
  describe "Git.remote/1 integration" do
    test "fresh repo has no remotes" do
      config = setup_repo()
      assert {:ok, []} = Git.remote(config: config)
    end
    ...
    test "remove remote returns :done" do
      ...
      assert {:ok, :done} = Git.remote(config: config, remove: "origin")
    end
    ...
    test "removed remote no longer appears in listing" do
      ...
      Git.remote(config: config, remove: "origin")
    ...
    test "removing non-existent remote returns error" do
      config = setup_repo()
      assert {:error, _} = Git.remote(config: config, remove: "nonexistent")
    end
    ...
    test "verbose: false lists remote names without URLs" do
      ...
      assert {:ok, [%Remote{name: "origin", fetch_url: nil, push_url: nil}]} =
               Git.remote(config: config, verbose: false)
    end
  end
end
