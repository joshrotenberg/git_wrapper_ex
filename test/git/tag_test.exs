defmodule Git.TagTest do
  ...
  describe "Git.tag/1 integration" do
    ...
    test "create lightweight tag returns :done" do
      {config, _dir} = setup_repo()
      assert {:ok, :done} = Git.tag(config: config, create: "v1.0.0")
    end

    test "created lightweight tag appears in listing" do
      {config, _dir} = setup_repo()
      Git.tag(config: config, create: "v1.0.0")
    ...
    test "create annotated tag returns :done" do
      ...
      assert {:ok, :done} =
               Git.tag(config: config, create: "v2.0.0", message: "release 2.0")
    end
    ...
    test "delete tag returns :done" do
      ...
      assert {:ok, :done} = Git.tag(config: config, delete: "v1.0.0")
    end

    test "deleted tag no longer appears in listing" do
      {config, _dir} = setup_repo()
      Git.tag(config: config, create: "v1.0.0")
      Git.tag(config: config, delete: "v1.0.0")
    ...
    test "deleting non-existent tag returns error" do
      {config, _dir} = setup_repo()
      assert {:error, _} = Git.tag(config: config, delete: "nonexistent")
    end
    ...
    test "multiple tags all appear in listing" do
      ...
      Git.tag(config: config, create: "v1.0.0")
      Git.tag(config: config, create: "v2.0.0", message: "release 2.0")
      Git.tag(config: config, create: "v3.0.0")
    ...
    test "create tag at specific ref" do
      ...
      assert {:ok, :done} = Git.tag(config: config, create: "v0.1.0", ref: hash)
