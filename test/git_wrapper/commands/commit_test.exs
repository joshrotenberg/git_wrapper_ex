defmodule GitWrapper.CommitTest do
  use ExUnit.Case, async: true

  alias GitWrapper.CommitResult
  ...
  describe "commit with staged file" do
    test "returns a CommitResult with correct fields", %{tmp_dir: tmp_dir, config: config} do
      ...
      assert {:ok, %CommitResult{} = result} =
               GitWrapperEx.commit("add hello file", config: config)
  ...
  describe "commit with all: true" do
    test "commits tracked modified files without explicit staging", %{tmp_dir: tmp_dir, config: config} do
      ...
      System.cmd(
        "git",
        ["-c", "user.name=Test User", "-c", "user.email=test@test.com", "commit", "-m", "add tracked"],
        cd: tmp_dir
      )
      ...
      assert {:ok, %CommitResult{} = result} =
               GitWrapperEx.commit("update tracked file", config: config, all: true)
  ...
  describe "commit with allow_empty: true" do
    test "creates a commit with no file changes", %{config: config} do
      assert {:ok, %CommitResult{} = result} =
               GitWrapperEx.commit("empty commit", config: config, allow_empty: true)
  ...
  describe "commit with amend: true" do
    test "amends the previous commit", %{tmp_dir: tmp_dir, config: config} do
      ...
      {:ok, _} = GitWrapperEx.commit("original message", config: config)

      assert {:ok, %CommitResult{} = result} =
               GitWrapperEx.commit("amended message", config: config, amend: true, allow_empty: true)
  ...
  describe "commit failure" do
    test "returns error when nothing to commit", %{config: config} do
      assert {:error, {output, exit_code}} =
               GitWrapperEx.commit("should fail", config: config)
