defmodule GitWrapper.InitTest do
  use ExUnit.Case, async: true

  alias GitWrapper.Commands.Init
  ...
  describe "GitWrapper.Commands.Init args/1" do
  ...
  describe "GitWrapper.Commands.Init parse_output/2" do
  ...
  describe "GitWrapperEx.init/1" do
    test "initializes a new repository in the working directory", %{config: config} do
      assert {:ok, :done} = GitWrapperEx.init(config: config)
    ...
    test "initializes a bare repository with bare: true", %{tmp_dir: tmp_dir} do
      ...
      assert {:ok, :done} = GitWrapperEx.init(bare: true, config: config)
    ...
    test "initializes a repository at a given path", %{tmp_dir: tmp_dir, config: config} do
      ...
      assert {:ok, :done} = GitWrapperEx.init(path: sub_dir, config: config)
