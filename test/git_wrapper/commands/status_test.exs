defmodule GitWrapper.StatusTest do
  use ExUnit.Case, async: true

  alias GitWrapper.Config
  alias GitWrapper.Status
  ...
  describe "clean repository" do
    test "returns ok with empty entries and correct branch", %{config: config} do
      assert {:ok, %Status{} = status} = GitWrapperEx.status(config: config)
  ...
  describe "multiple changes" do
    test "shows multiple entries for different states", %{tmp_dir: tmp_dir, config: config} do
      ...
      System.cmd(
        "git",
        ["-c", "user.name=Test", "-c", "user.email=test@test.com", "commit", "-m", "add existing"],
        cd: tmp_dir
      )
