defmodule GitWrapper.DiffTest do
  use ExUnit.Case, async: true

  alias GitWrapper.Diff
  alias GitWrapper.Commands.Diff, as: DiffCmd
  ...
  describe "Commands.Diff.args/1" do
    ...
    test "ref and path together" do
      assert DiffCmd.args(%DiffCmd{ref: "HEAD~1", path: "lib/"}) == ["diff", "HEAD~1", "--", "lib/"]
    end
  ...
  describe "GitWrapperEx.diff/1 integration" do
    ...
    test "unstaged changes appear in working-tree diff" do
      ...
      assert {:ok, diff} = GitWrapperEx.diff(config: config)
      assert diff.raw != ""
    end
    ...
    test "staged diff with stat: true returns file stats" do
      ...
      assert {:ok, diff} = GitWrapperEx.diff(config: config, staged: true, stat: true)
      assert diff.total_insertions > 0
    ...
    test "staged diff with no staged changes returns empty diff" do
      ...
      assert {:ok, %Diff{files: [], total_insertions: 0}} =
               GitWrapperEx.diff(config: config, staged: true, stat: true)
    end
    ...
    test "diff with ref compares against that ref" do
      ...
      assert {:ok, diff} = GitWrapperEx.diff(config: config, ref: "HEAD~1", stat: true)
      assert diff.total_insertions > 0
    end
  end
end
