defmodule GitWrapper.BranchTest do
  use ExUnit.Case, async: true

  alias GitWrapper.Branch
  alias GitWrapper.Commands.Branch, as: BranchCmd
  ...
  describe "GitWrapperEx.branch/1 integration" do
    test "lists the initial branch" do
      {config, _dir} = setup_repo()
      assert {:ok, branches} = GitWrapperEx.branch(config: config)
      assert length(branches) == 1
    ...
    test "create branch returns :done" do
      {config, _dir} = setup_repo()
      assert {:ok, :done} = GitWrapperEx.branch(config: config, create: "feat/new")
    end

    test "created branch appears in listing" do
      {config, _dir} = setup_repo()
      GitWrapperEx.branch(config: config, create: "feat/new")
    ...
    test "delete branch returns :done" do
      {config, _dir} = setup_repo()
      GitWrapperEx.branch(config: config, create: "to-delete")

      assert {:ok, :done} = GitWrapperEx.branch(config: config, delete: "to-delete")
    end

    test "deleted branch no longer appears in listing" do
      {config, _dir} = setup_repo()
      GitWrapperEx.branch(config: config, create: "to-delete")
      GitWrapperEx.branch(config: config, delete: "to-delete")
    ...
    test "deleting non-existent branch returns error" do
      {config, _dir} = setup_repo()
      assert {:error, _} = GitWrapperEx.branch(config: config, delete: "nonexistent")
    end

    test "multiple branches all appear in listing" do
      {config, _dir} = setup_repo()
      GitWrapperEx.branch(config: config, create: "feat/a")
      GitWrapperEx.branch(config: config, create: "feat/b")
