defmodule GitWrapper.Commands.Commit do
  @moduledoc """
  Implements the `GitWrapper.Command` behaviour for `git commit`.
  ...
  ## Examples
      ...
      iex> args = GitWrapper.Commands.Commit.args(%GitWrapper.Commands.Commit{message: "test", all: true})
      iex> args
      ["commit", "-m", "test", "-a"]
  ...
  On success (exit code 0), parses the output into a `GitWrapper.CommitResult`
  struct. On failure, returns `{:error, {stdout, exit_code}}`.
  """
  @spec parse_output(String.t(), non_neg_integer()) ::
          {:ok, GitWrapper.CommitResult.t()} | {:error, {String.t(), non_neg_integer()}}
  @impl true
  def parse_output(stdout, 0), do: {:ok, GitWrapper.CommitResult.parse(stdout)}
